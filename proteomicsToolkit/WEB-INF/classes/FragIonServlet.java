...
public class FragIonServlet extends HttpServlet
{
   ...
   private String sequence,
      ...
      cterm,
      addModifType,
      addModifVal, outputSeq, charge, aCB, bCB, cCB, xCB, yCB, zCB;
   ...
   private int invalidInput = 0,        //  0 == OK, 1 == invalid input (set by parseInputSequence() )
     invalidAddMods = 0;        //  0 == OK, 1 == invalid additinal mod input (set by addModAdder() )
   ...
   //  holds modifications entered by user for output with results
   StringBuffer modifBuff = new StringBuffer ("");
   ...
   public void Output (String seq)
   {
      ...
      if (invalidInput == 0)
      {
         ...
         //  STEP1 -->  calculate mass and output
         //  STEP2 -->  calculate fragment ions and output
         ...
         for (int i = 0; i < allSequences.size(); i++)
         {
            ...
            // output sequence
            out.println("Sequence: <B>" + outputBy50(tempSeq) + "</B>, &nbsp; ");

            //  output pI
            out.println("pI: <B>" + roundFxn(calc_pI (tempSeq)) + "</B><BR>");
            ...
            if (aCB != null ||
                bCB != null ||
                cCB != null || xCB != null || yCB != null || zCB != null)
            {
               out.println("<H3>Fragment Ion Table, ");

               if (massType.equals("monoRB"))
                  out.println("monoisotopic masses");
               if (massType.equals("avgRB"))
                  out.println("average masses");
               out.println("</H3>");
            ...
            out.println("<H3>Mass/Charge Table</H3>");
            out.println(massTable);

            out.println("<TABLE><TR><TD>" + modifBuff + "</TD></TR></TABLE>");
            out.println("</CENTER>");
   ...
   public String massTblOut (String seq)
   {
      ...
      tableBuff.append("<TABLE BORDER WIDTH = 25%>");
      ...
      //  monoisotopic weight
      origMassDblMono = lookupWeight (completeSeq, 0);  // saves the non-charged mono mass

      //  average weight
      origMassDblAvg = lookupWeight (completeSeq, 1);   // saves the non-charged avg mass

/* 06/17/2003 JEng - add modifications to mass table */

      //  if there is n-terminus modification
      if (nterm.length() > 0)
      {
         origMassDblMono += new Double (nterm).doubleValue ();
         origMassDblAvg += new Double (nterm).doubleValue ();
      }

      //  if there is c-terminus modification
      if (cterm.length() > 0)
      {
         origMassDblMono += new Double (cterm).doubleValue ();
         origMassDblAvg += new Double (cterm).doubleValue ();
      }

      if (addModifVal.length() > 0)    // there are additional modifications present
      {
         for (int j = 0; j < addModifTypeVec.size(); j++)
         {
            //  fetch value to be added (throws NumberFormatException if value is not a number)
            double addMod = new Double (String.valueOf((String) addModifValVec.elementAt(j))).doubleValue();
            String modCharString = (String) addModifTypeVec.elementAt(j);
	    modCharString = modCharString.trim();  //added by nzhang,possible space char in it.
            char modChar = modCharString.charAt(0);    // this could either be a residue or index


            for (int k = 0; k < seq.length(); k++)
            {
               String indexVal = String.valueOf(k + 1);
               char indexValchar = indexVal.charAt(0);

               //if (modChar == indexValchar)     //  check for matching indices in sequences
	       
	       /*11/21/2007 NZhang fix the bug, if position > 9, using modChar gives wrong index*/
	       if(modCharString.equals(indexVal)) //check for matching indices in sequences
               {
                  origMassDblMono += addMod;
                  origMassDblAvg += addMod;
               }

               if (modChar == seq.charAt(k))   //  check for matching letters in sequences
               {
                  origMassDblMono += addMod;
                  origMassDblAvg += addMod;
               }
      ...
      massStr1 = roundFxn(origMassDblMono).toString();
      massStr2 = roundFxn(origMassDblAvg).toString();

      tableBuff.append("<TR><TH ALIGN = LEFT><NOBR>(M)</TH></NOBR><TD ALIGN=RIGHT><TT>" +
          massStr1 + "</TT></TD><TD ALIGN=RIGHT><TT>" + massStr2 + "</TT></TD></TR>\n");
      ...
      tableBuff.append("<TR><TH ALIGN = LEFT><NOBR>(M+H)<SUP>+</SUP></NOBR></TH><TD ALIGN=RIGHT><TT>"
          + massStr1 + "</TT></TD><TD ALIGN=RIGHT><TT>" + massStr2 + "</TT></TD></TR>\n");
      ...
      tableBuff.append("<TR><TH ALIGN = LEFT><NOBR>(M+2H)<SUP>2+</SUP></NOBR></TH><TD ALIGN=RIGHT><TT>"
          + massStr1 + "</TT></TD><TD ALIGN=RIGHT><TT>" + massStr2 + "</TT></TD></TR>\n");
      ...
      tableBuff.append("<TR><TH ALIGN = LEFT><NOBR>(M+3H)<SUP>3+</SUP></NOBR></TH><TD ALIGN=RIGHT><TT>"
          + massStr1 + "</TT></TD><TD ALIGN=RIGHT><TT>" + massStr2 + "</TT></TD></TR>\n");
      ...
      tableBuff.append("<TR><TH ALIGN = LEFT><NOBR>(M+4H)<SUP>4+</SUP></NOBR></TH><TD ALIGN=RIGHT><TT>"
          + massStr1 + "</TT></TD><TD ALIGN=RIGHT><TT>" + massStr2 + "</TT></TD></TR>\n");

      tableBuff.append("</TABLE><P>\n");
   ...
   private String AAStr = "",
      ...
      bIonMassStr = "",
      cIonMassStr = "",
      xIonMassStr = "", yIonMassStr = "", zIonMassStr = "", tableStr = "";
   ...
   public String ionTblOut (String s)
   {
      ...
      outputBuff.append("<B><TABLE BORDER CELLPADDING=5><TR BGCOLOR=#FFFFCC><TD BGCOLOR=><PRE>"
          + padCenter ("Seq", 3) + padLeft ("#", 3));

      //  create heading for each ion selected
      if (aCB != null)
         outputBuff.append("<FONT COLOR=BROWN>" + padCenter ("A", 11) + "</FONT>");
      if (bCB != null)
         outputBuff.append("<FONT COLOR=BLUE>" + padCenter ("B", 11) + "</FONT>");
      if (cCB != null)
         outputBuff.append("<FONT COLOR=GREEN>" + padCenter ("C", 11) + "</FONT>");
      if (xCB != null)
         outputBuff.append("<FONT COLOR=#806517>" + padCenter ("X", 11) + "</FONT>");
      if (yCB != null)
         outputBuff.append("<FONT COLOR=RED>" + padCenter ("Y", 11) + "</FONT>");
      if (zCB != null)
         outputBuff.append("<FONT COLOR=PURPLE>" + padCenter ("Z", 11) + "</FONT>");


      if (charge.equals("1"))
         outputBuff.append(padCenter ("  # (+1)", 3));
      if (charge.equals("2"))
         outputBuff.append(padCenter ("  # (+2)", 3));
      if (charge.equals("3"))
         outputBuff.append(padCenter ("  # (+3)", 3));

      outputBuff.append("</B><BR>");

      outputBuff.append("</TD></TR><TR><TD><PRE>\n");
      ...
      for (int i = 0; i < s.length(); i++)
      {
         ...
         if (tokAmino.hasMoreTokens () && tokNumIncr.hasMoreTokens ())
         {
            ...
            outputBuff.append(padCenter (tokAmino.nextToken (), 3) + padLeft (tokNumIncr.nextToken (), 3));
         }

         if (aCB != null && tokA.hasMoreTokens ())
         {
            outputBuff.append("<FONT COLOR=BROWN>" + padLeft (tokA.nextToken (), 11) + "</FONT>");
         }
         if (bCB != null && tokB.hasMoreTokens ())
         {
            outputBuff.append("<FONT COLOR=BLUE>" + padLeft (tokB.nextToken (), 11) + "</FONT>");
         }
         if (cCB != null && tokC.hasMoreTokens ())
         {
            if (i == s.length() - 1)
            {
               ...
               outputBuff.append("<FONT COLOR=GREEN>" + padCenter ("-", 11) + "</FONT>");
            }
            else
               outputBuff.append("<FONT COLOR=GREEN>" + padLeft (tokC.nextToken (), 11) + "</FONT>");
         }
         if (xCB != null && tokX.hasMoreTokens ())
         {
            if (i == 0)
            {
               ...
               outputBuff.append("<FONT COLOR=#806517>" + padCenter ("-", 11) + "</FONT>");
            }
            else
               outputBuff.append("<FONT COLOR=#806517>" + padLeft (tokX.nextToken (), 11) + "</FONT>");
         }
         if (yCB != null && tokY.hasMoreTokens ())
         {
            outputBuff.append("<FONT COLOR=RED>" + padLeft (tokY.nextToken (), 11) + "</FONT>");
         }
         if (zCB != null && tokZ.hasMoreTokens ())
         {
            outputBuff.append("<FONT COLOR=PURPLE>" + padLeft (tokZ.nextToken (), 11) + "</FONT>");
         }

         if (tokNumDecr.hasMoreTokens ())
         {
            outputBuff.append(padLeft (tokNumDecr.nextToken (), 3));
         }
...
//  outputVec.addElement(outputBuff.toString());
   ...
   private String ntermMod, ctermMod, completeABC, completeXYZ, IonMassStr;

   private StringBuffer str = new StringBuffer ("");

   private double termDblABC, termDblXYZ, addModDblABC, addModDblXYZ, IonMass;
   ...
   public void calcIon (String s)
   {
      ...
      if (addModifVal.length() > 0)    // there are additional modifications present
      {
         ...
         if (tokType.hasMoreTokens () && tokVal.hasMoreTokens ())
            modifBuff.append("Modifications: <BR>");
         ...
         while (tokType.hasMoreTokens () && tokVal.hasMoreTokens ())
         {
            ...
            String val = tokVal.nextToken ();

            addModifTypeVec.addElement (type.toUpperCase ());
            addModifValVec.addElement (val);

            modifBuff.append(padCenter ("*", 5) + "To residue " + type + " added the value " + val + "<BR>");
//        if(tokType.hasMoreTokens() && tokVal.hasMoreTokens() )
//          modifBuff.append("<BR>");
         }
      }
      ...
      for (int i = 0; i < s.length(); i++)
      {
         ...
         completeABC = s.substring (0, (i + 1));

         //  fragment ion string for x, y, or z ion -- must readd C-terminal -OH
         completeXYZ = s.substring (i, s.length());

         addModAdder (s, i);    //  pass it the index of sequence function is at

         //  this part is specific for each ion

         if (aCB != null)
         {
            IonMass = divCharge ((lookupWeight (completeABC + "h") - lookupWeight ("co")) + termDblABC + addModDblABC);
         ...
         if (bCB != null)
         {
            IonMass = divCharge (lookupWeight (completeABC + "h") + termDblABC + addModDblABC);
            str = roundFxn (IonMass);
            bIonMassBuff.append("!");
         ...
         if (cCB != null)
         {
            IonMass = divCharge ((lookupWeight (completeABC) + lookupWeight ("nhhhh")) + termDblABC + addModDblABC);
            str = roundFxn (IonMass);
            cIonMassBuff.append("!");
            cIonMassBuff.append(str);
         }
         if (xCB != null)       //  skip the first ion calculation
         {
            IonMass = divCharge (lookupWeight (completeXYZ) + lookupWeight ("cooh") + termDblXYZ + addModDblXYZ);
            str = roundFxn (IonMass);
            xIonMassBuff.append("!");
         ...
         if (yCB != null)
         {
            IonMass = divCharge ((lookupWeight (completeXYZ) + lookupWeight ("ohhh")) + termDblXYZ + addModDblXYZ);
            str = roundFxn (IonMass);
            yIonMassBuff.append("!");
         ...
         if (zCB != null)
         {
            IonMass = divCharge ((lookupWeight (completeXYZ + "oh") - lookupWeight ("nh")) + termDblXYZ + addModDblXYZ);
            str = roundFxn (IonMass);
            zIonMassBuff.append("!");
   ...
   /*
    *  ADDITIONAL MODIFICATIONS ADDER
    */

   //  takes the index of sequence that the calcIon function is at
   // returns 0 if all is well, and 1 if the additional modifications entered are invalid (NumberFormatException thrown)
   public void addModAdder (String s,
                            int i)
   {
      termDblABC = 0;
      termDblXYZ = 0;
      ...
      try
      {
         ...
         /*  Add n-term or c-term modifications to the mass of a, b, and c ions */
         ...
         if (ntermMod.length() > 0)
         {
            ...
            if (i == 0 && termDblABC > 0.0)
               modifBuff.append("* N-terminus modification: " + termDblABC + "<BR>");
         }

         //  if there is c-terminus modification it is retrieved and added to LAST fragment
         if (ctermMod.length() > 0 && i == (s.length() - 1))
         {
            double cDbl = new Double (ctermMod).doubleValue ();
            termDblABC += cDbl;
            if (cDbl > 0.0)
               modifBuff.append("* C-terminus modification: " + cDbl + "<BR>");
         }


         //  Add n-term or c-term modifications to the mass of x,y, and z ions
         ...
         //  add additional modifications specified by user

/* 06/17/2003 JEng - fix how modifications are used; previous residue modifications incorrect */
/* 11/21/2007 NZhang - fix the bug if mod position is double digit e.g. 11 */
         ...
         for (int j = 0; j < addModifTypeVec.size(); j++)
         {
            ...
            double addMod = new Double (String.valueOf((String) addModifValVec.elementAt (j))).doubleValue ();
            String modCharString = (String) addModifTypeVec.elementAt (j);
	    modCharString = modCharString.trim();
            char modChar = modCharString.charAt(0);

            for (int k = 0; k < completeABC.length(); k++)
            {
               String indexABC = String.valueOf(k + 1);
               char indexABCchar = indexABC.charAt(0);

               //if (modChar == indexABCchar)     //  check for matching indices in sequences
	       if(modCharString.equals(indexABC)) // if the position > 9, charAt(0) won't work
                  addModDblABC += addMod;

               if (modChar == completeABC.charAt(k))   //  check for matching letters in sequences
                  addModDblABC += addMod;
            }

            for (int k = 0; k < completeXYZ.length(); k++)
            {
               String indexXYZ = String.valueOf(s.length() - k);
               char indexXYZchar = indexXYZ.charAt(0);

               //if (modChar == indexXYZchar)     //  check for matching indices in sequences
	       if(modCharString.equals(indexXYZ)) //if the position is two digit, modChar won't work
                  addModDblXYZ += addMod;

               if (modChar == completeXYZ.charAt(completeXYZ.length() - k - 1))       //  check for matching letters in sequences
                  addModDblXYZ += addMod;
            }
...
/* Z */ {3.55, 7.50, 0.0, 0.0, 0.0}
   ...
   public double calc_pI (String seq)
   {
      ...
      double charge, ph_mid, ph_min, ph_max, cter, nter, carg,  //  charge for arginines (R)
   ...
   public String padCenter (String content,
                            int padNum)
   {
   ...
   public String padLeft (String content,
                          int padNum)
   {
