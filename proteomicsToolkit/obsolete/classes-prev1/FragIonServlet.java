 ...
 // ************************** HELPER METHODS **************************
  ...
  public void Output(String seq)
  {
   ...
   if (invalidInput == 0)
   {
    ...
    for (int i = 0; i < allSequences.size(); i++)
    {
      ...
      // output sequence
      out.println("<B>Sequence:</B> " + outputBy50(tempSeq) + "<BR>");

      //  output pI
      out.println("<B>pI:</B> " + roundFxn(calc_pI(tempSeq)) + "<BR>");
      ...
      if (aCB != null ||
          ...
          zCB != null )
      {
        out.println("<H3>Fragment Ion Table</H3>");

        if (massType.equals("monoRB"))
          modifBuff.append("Mass type:  Monoisotopic <BR>");
        if (massType.equals("avgRB"))
          modifBuff.append("Mass type:  Average  <BR>");
   ...
   */
   ...
   public String massTblOut(String seq)
   {
    ...
    tableBuff.append("<TR><TH ALIGN = LEFT><NOBR>(M)</TH></NOBR><TD>" + massStr1 + "</TD><TD>" + massStr2 + "</TD></TR>");
    ...
    tableBuff.append("<TR><TH ALIGN = LEFT><NOBR>(M+H)<SUP>+</SUP></NOBR></TH><TD>" + massStr1 + "</TD><TD>" + massStr2 + "</TD></TR>");
    ...
    tableBuff.append("<TR><TH ALIGN = LEFT><NOBR>(M+2H)<SUP>2+</SUP></NOBR></TH><TD>" + massStr1 + "</TD><TD>" + massStr2 + "</TD></TR>");
    ...
    tableBuff.append("<TR><TH ALIGN = LEFT><NOBR>(M+3H)<SUP>3+</SUP></NOBR></TH><TD>" + massStr1 + "</TD><TD>" + massStr2 + "</TD></TR>");
    ...
    tableBuff.append("<TR><TH ALIGN = LEFT><NOBR>(M+4H)<SUP>4+</SUP></NOBR></TH><TD>" + massStr1 + "</TD><TD>" + massStr2 + "</TD></TR>");
  ...
  outputBuff.append("<B><TABLE BORDER><TR><TD><PRE>" + padCenter("Seq", 3) + padLeft("#",3));

  //  create heading for each ion selected
  if (aCB != null)
    outputBuff.append("<FONT COLOR = #990033>" + padCenter("a",11) + "</FONT>");
  if (bCB != null)
    outputBuff.append("<FONT COLOR = #ff6633>" + padCenter("b",11) + "</FONT>");
  if (cCB != null)
    outputBuff.append("<FONT COLOR = #006633>" + padCenter("c",11) + "</FONT>");
  if (xCB != null)
    outputBuff.append("<FONT COLOR = #006666>" + padCenter("x",11) + "</FONT>");
  if (yCB != null)
    outputBuff.append("<FONT COLOR = #000066>" + padCenter("y",11) + "</FONT>");
  if (zCB != null)
    outputBuff.append("<FONT COLOR = #663366>" + padCenter("z",11) + "</FONT>");


  if (charge.equals("1"))
    outputBuff.append(padCenter("(+1)",4));
  if (charge.equals("2"))
    outputBuff.append(padCenter("(+2)",4));
  if (charge.equals("3"))
    outputBuff.append(padCenter("(+3)",4));

  outputBuff.append("</B><BR>");

  outputBuff.append(padCenter("---",3) + padLeft("---",3));

  if (aCB != null)
    outputBuff.append(padCenter("-----------",11));
  if (bCB != null)
    outputBuff.append(padCenter("-----------",11));
  if (cCB != null)
    outputBuff.append(padCenter("-----------",11));
  ...
  outputBuff.append(padCenter("----",4) + "<BR>");
  ...
  for (int i = 0; i < s.length(); i++)
  {
   ...
   if (tokAmino.hasMoreTokens() && tokNumIncr.hasMoreTokens())
   {
     ...
     outputBuff.append("<PRE>" + padCenter(tokAmino.nextToken(),3) + padLeft(tokNumIncr.nextToken(),3));
   }

   if (aCB != null && tokA.hasMoreTokens())
   {
     outputBuff.append("<FONT COLOR = #990033>" + padLeft(tokA.nextToken(),11) + "</FONT>");
   }
   if (bCB != null && tokB.hasMoreTokens())
   {
     outputBuff.append("<FONT COLOR = #ff6633>" + padLeft(tokB.nextToken(),11) + "</FONT>");
   }
   if (cCB != null && tokC.hasMoreTokens() )
   {
     if (i == s.length()-1)
     {
       ...
       outputBuff.append("<FONT COLOR = #006633>" + padCenter("-",11) + "</FONT>");
     }
     else
       outputBuff.append("<FONT COLOR = #006633>" + padLeft(tokC.nextToken(),11) + "</FONT>");
   }
   if (xCB != null && tokX.hasMoreTokens())
   {
     if (i == 0)
     {
       ...
       outputBuff.append("<FONT COLOR = #006666>" + padCenter("-",11) + "</FONT>");
     }
     else
       outputBuff.append("<FONT COLOR = #006666>" + padLeft(tokX.nextToken(),11) + "</FONT>");
   }
   if (yCB != null && tokY.hasMoreTokens() )
   {
     outputBuff.append("<FONT COLOR = #000066>" + padLeft(tokY.nextToken(),11) + "</FONT>");
   }
   if (zCB != null && tokZ.hasMoreTokens())
   {
     outputBuff.append("<FONT COLOR = #663366>" + padLeft(tokZ.nextToken(),11) + "</FONT>");
   }

   if (tokNumDecr.hasMoreTokens() )
   {
     outputBuff.append(padLeft(tokNumDecr.nextToken(),4)  );
   }
...
//          modifBuff.append("<BR>");
  ...
  public void addModAdder (String s, int i)
  {
      termDblABC = 0;
      termDblXYZ = 0;
      addModDblABC = 0;
      ...
      try {
          ...
          for (int j = 0; j < addModifTypeVec.size(); j++)
          {
            ...
            //  check for matching letters in sequences
            if (completeABC.indexOf(((String)addModifTypeVec.elementAt(j)).charAt(0)) != (-1))  //  there is a match
            {
              addModDblABC += addMod;
            }
            if ( completeXYZ.indexOf  ( ((String)addModifTypeVec.elementAt(j)).charAt(0)) != (-1))  //  there is a match
            {
              addModDblXYZ += addMod;
            }

            //  check for matching indices in sequences
            for (int k = 0; k < completeABC.length(); k++)
            {
              String indexABC = String.valueOf(k+1);
              if ( ( (String)addModifTypeVec.elementAt(j) ).equalsIgnoreCase(indexABC) )
                addModDblABC += addMod;
                
            }
            for (int k = 0; k < completeXYZ.length(); k++)
            {
              String indexXYZ = String.valueOf(k+i+1);
              if ( ( (String)addModifTypeVec.elementAt(j) ).equalsIgnoreCase(indexXYZ) )
                addModDblXYZ += addMod;
            }
...
*/
  ...
  public String padLeft (String content, int padNum)
  {
